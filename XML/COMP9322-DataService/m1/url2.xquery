...
<html>
    ...
    <body>
        ...
        <tbody>
            {
                for $postcode in doc("licence_q1.xml")/year/quarter[@name = $ex_quarter]/postcode[@name = $ex_postcode]
                return
                <tr>
                    <th style="text-align:left;">{data($postcode/@name)}</th>
                    {
                        for $classType in $postcode/class-and-type
                        return 
